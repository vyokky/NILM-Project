...
class FHMM_Relaxed(object):
    ...
    Authors: Mingjun Zhong, Chaoyun Zhang
    Attributes
    ----------
    model :
    ...
    def __init__(self, NosOfIters=3, index=None):
        ...
        self.index = index
        # sampling seconds
        self.sample_seconds = 6
        
        # number of iterations
        self.NosOfIters = NosOfIters
    ...
    def disaggregate_convnet_fhmm_chunk(self):
        """In-memory disaggregation.
        ...
        # constants
        if self.index:
            mains_chunk = self.model['mains'][self.index[0]:self.index[1]]
        else:
            mains_chunk = self.model['mains']
        ...
        for iters in range(self.NosOfIters):
            # infer the model variables
            prediction = self.disaggregate_fhmm_convnet(mains_chunk)
    ...
    def disaggregate_fhmm_convnet(self, mains_chunk):
        ...
        nosOfTimePoints = mains_chunk.size
        ...
        with Model("composite model") as M:
            ...
            for i, appliance in enumerate(self.individual):
                ...
                diffLatent = Expr.sub(latentVariableOfAppliance[appliance],
                                      Expr.mul(1.0 * np.array(self.individual[appliance]['mean']).
                ...
                if i == 0:
                    ConvNetPosterior = Expr.sum(Expr.mulElm(stateVariableOfAppliance[appliance],
                                                            -np.log(np.maximum(1e-300,
                                                                    self.individual[appliance]['softmax'].T))))
                else:
                    ConvNetPosterior = Expr.add(ConvNetPosterior,
                                                Expr.sum(Expr.mulElm(stateVariableOfAppliance[appliance],
                                                                     -np.log(np.maximum(1e-300,
                                                                             self.individual[appliance]['softmax'].T)))))
            ...
            rqc_logDataLikelihood = M.constraint(
                Expr.hstack(self.varModel, delta,
                            Expr.sub(DenseMatrix(
                                ...
                                mains_chunk.ravel().reshape(1, nosOfTimePoints).tolist()),
            ...
            inferred_appliance_mains_energy = pd.DataFrame()
            inferred_latent_energy = pd.DataFrame()
            ...
            for (i, appliance) in enumerate(self.individual):
                ...
                inferred_appliance_mains_energy[appliance] = \
                    np.dot(np.array(self.individual[appliance]['mean']).flatten(),
    ...
    def disaggregate_fixedPrecision(self, mains_chunk):
        ...
        with Model("composite model") as M:
            ...
            ###### Reading the inference results ##############################
            inferred_appliance_mains_energy = pd.DataFrame(index=np.array(range(mains_chunk.values.size)))
            # print (mains_chunk.index)
            # print (type(mains_chunk))
            inferred_latent_energy = pd.DataFrame(index=np.array(range(mains_chunk.values.size)))
            inferred_states = OrderedDict()
            inferred_relaxedStates = OrderedDict()
            inferred_total_energy = OrderedDict()
            prediction = {}
    ...
    def estimate_noisevariance(self, mains_chunk, prediction):
        ...
        self.varModel = (self.beta + 
           0.5*np.sum((mains_chunk.ravel().reshape(1,nosOfTimePoints)
    ...
    def objective(self, mains_chunk, prediction):
        ...
        optimalObjective = optimalObjective \
            ...
            np.sum((mains_chunk.ravel().reshape(1,nosOfTimePoints)
           - inferred_latent_energy['inferred mains'].values 
           - inferred_latent_energy['piecewise noise'].values )**2)\
    ...
    def import_model_cPickle(self, meterlist, model):
        """Loads learned model from file.
                Required to be overridden for learned models to persist.
                ...
                ----------
                filename : str path to file to load model from
                           mains reading should be included and has a type as numpy.array
                meterlist: list includes required submeters
        """
        ...
        for meter in meterlist:
            if meter in self.model.keys():
                individual[meter] = copy.deepcopy(self.model[meter])
                if self.index:
                    individual[meter]['softmax']=individual[meter]['softmax'][self.index[0]:self.index[1]]
                individual[meter]['numberOfStates'] = self.model[meter]['mean'].size
            else:
                print("The meter {0} is not in the trained model".format(meter))
            print("The trained meter: {0}".format(meter))
        self.individual = individual
    ...
    def checkconstraints(self,prediction):
        ...
        # Estimate the model noise variance
        objModel = np.sum((self.mains_chunk.ravel().reshape(1,nosOfTimePoints)
                           - inferred_latent_energy['inferred mains'].values 
                           - inferred_latent_energy['piecewise noise'].values )**2)
